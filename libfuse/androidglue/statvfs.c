 ...
 * 
 * Modified cvt_statfs_to_statvfs() for some members not exist in android
 */
...
cvt_statfs_to_statvfs(const struct statfs *fsp, struct statvfs *vfsp)
{
	...
	vfsp->f_bsize = fsp->f_bsize;
	//vfsp->f_frsize = fsp->f_bsize; /* XXX */
	vfsp->f_frsize = fsp->f_frsize;
    ...
    int *fsid = (int *)&(fsp->f_fsid);
    vfsp->f_fsid   = (fsid[0] << sizeof(fsid[0])) | fsid[1];
